...
```
server.js          — Express: GET / injects GOOGLE_MAPS_API_KEY into index.html; serves /public static
public/
  index.html       — UI structure; loads map.js, app.js, then Google Maps (callback=initApp)
  ...
  js/
    ...
    geocoder.js    — Unused (replaced by Places Autocomplete)
```
...
5. `TransitHeatmap.setData(points)` → `draw()` renders radial-gradient circles on a canvas positioned via `OverlayView.getProjection()`. `draw()` is also called on `map idle` so the canvas repositions after pan/zoom.
